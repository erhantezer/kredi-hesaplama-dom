...
const hesaplaBtn = document.querySelector(".btn.btn-dark");
...
hesaplaBtn.addEventListener("click", (e) => {
    ...
    if (!select.value || !vade.value || !tutar.value) {
        alert("Please all value enter")
    }

    //! tıklanınca hesaplama yapması gerek aşağıdaki gibi
    const faiz = oran / 100;
    taksit = (tutar.value * (faiz * (1 + faiz) ** vade.value)) / ((1+faiz) ** vade.value - 1);
    console.log(taksit);

    //! bulunanları tablo şeklinde almamız gerek bu yüzden fonksiyon çağıracağız
    sonuclariGoster()
});
